...
export default function GradingAssistant() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        sessionStorage.setItem("grading_password", password)
      } else {
  ...
  const handleLogout = () => {
    ...
    setPassword("")
    sessionStorage.removeItem("grading_password")
    clearResults()
  }

  // Check for stored session on mount
  if (!isAuthenticated && typeof window !== "undefined") {
    const stored = sessionStorage.getItem("grading_password")
    if (stored && !password) {
      setPassword(stored)
      setIsAuthenticated(true)
    }
        ...
        >
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 space-y-6">
            <div className="text-center">
              <div className="w-14 h-14 bg-gradient-to-br from-teal-500 to-emerald-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-teal-500/20">
                <Lock className="h-7 w-7 text-white" />
              </div>
              <h1 className="text-2xl font-bold text-slate-900">SLHS 303</h1>
              <p className="text-slate-500 mt-1">Grading Assistant</p>
              <p className="text-xs text-slate-400 mt-1">Instructor access only</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm"
                  autoFocus
                />
              </div>
